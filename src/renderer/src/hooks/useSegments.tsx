...
function useSegments({ filePath, workingRef, setWorking, setProgress, videoStream, fileDuration, getRelevantTime, maxLabelLength, checkFileOpened, invertCutSegments, segmentsToChaptersOnly, timecodePlaceholder, parseTimecode, appendFfmpegCommandLog, fileDurationNonZero, mainFileMeta, seekAbs, activeVideoStreamIndex, activeAudioStreamIndexes, handleError, showGenericDialog, simpleMode, ffmpegHwaccel }: {
  ...
  const alignSegmentTimesToKeyframes = useCallback(async () => {
    ...
    try {
      ...
      const frameDuration = 1 / (getStreamFps(videoStream) || 1000);
      ...
      await modifySelectedSegmentTimes(async (segment) => {
        ...
        if (newSegment.end !== undefined) {
          ...
          if (startOrEnd.includes('start')) {
            newSegment.start = Math.min(newSegment.start, newSegment.end - frameDuration * 0.99); // don't know how ffmpeg interprets cuts between frames
          } else {
            newSegment.end = Math.max(newSegment.start + frameDuration * 0.99, newSegment.end);
          }
        }
  ...
  }, [videoStream, workingRef, setWorking, filePath, modifySelectedSegmentTimes, fileDuration, handleError]);
